...
?>
<p><a href="<?php echo $or->url('series.php'); ?>">Add another rehearsal series</a></p>
...
foreach ($serieslist as $series) {
    $actions = array();
    if ($series->deleted) {
        $actions[] = $output->action_button($or->url('serieslist.php', false),
                array('undelete' => $series->id), 'Restore');
        $extrarowclass = ' deleted';

    } else {
        $actions[] = '<a href="' . $or->url('series.php?id=' . $series->id, false) . '">Edit</a>';
